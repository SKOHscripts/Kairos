...
4. **Tâche mère + deux sous-tâches** (« Rédiger la documentation », puis
   « Documentation : plan » et « Documentation : rédaction ») : seules les
   feuilles portent `estimated_minutes`/`deadline`/planification ; la mère n'en
   a pas (avancement n/m affiché côté rendu).
...
6. **Inbox « À traiter »** (« Idée : automatiser le rapport hebdomadaire ») :
   **ni `priority` ni `fibonacci_points`** : seule tâche du jeu d'exemple sans
   ...
   page tant qu'elle n'est pas qualifiée, logique portée par `ordonnancement` et
   non par ce module).
