...
def test_get_home_toc_sits_right_after_en_bref() -> None:
    """#13.5 : le sommaire (occurrence unique, même mise en page petit/grand écran)
    tombe entre la section « En bref » du README rendu et la section suivante —
    pas juste avant tout l'article."""
    client = TestClient(main.app)
    resp = client.get("/")
    assert resp.status_code == 200
    text = resp.text
    en_bref_end = text.index("id=\"en-bref\"")
    toc_pos = text.index("class=\"home-toc panel\"")
    next_section_pos = text.index("id=\"telecharger-lapplication-windowslinuxandroid\"")
    assert en_bref_end < toc_pos < next_section_pos
    # Une seule occurrence du sommaire dans la page (plus de copie sidebar/inline).
